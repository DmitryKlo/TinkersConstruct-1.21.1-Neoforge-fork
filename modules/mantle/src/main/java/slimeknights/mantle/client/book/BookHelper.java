...
public class BookHelper {
  ...
  public static String getCurrentSavedPage(@Nullable ItemStack item) {
    if (item != null) {
      if (!item.isEmpty() && StackDataHelper.hasTag(item)) {
        CompoundTag bookNBT = StackDataHelper.getOrCreateTag(item).getCompound(BOOK_COMPOUND).getCompound(BOOK_DATA_COMPOUND);
  ...
  public static void writeSavedPageToBook(ItemStack stack, String currentPage) {
    CompoundTag compoundNBT = StackDataHelper.getOrCreateTag(stack);
    ...
    StackDataHelper.setTag(stack, compoundNBT);
  }
}
