...
public interface RecordLoadable<T> extends Loadable<T> {
  /** Creates a minimal no-field loadable returning a fixed value. Intended for marker modules with no JSON fields. */
  static <R> RecordLoadable<R> singleton(R value) {
    return new RecordLoadable<>() {
      @Override
      ...
      @Override
      public R decode(net.minecraft.network.FriendlyByteBuf buffer, TypedMap context) {
        return value;
      }

      @Override
      public void encode(net.minecraft.network.FriendlyByteBuf buffer, R value) {}
    };
  }
