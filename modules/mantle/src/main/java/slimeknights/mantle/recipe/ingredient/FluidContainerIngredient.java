...
public class FluidContainerIngredient extends AbstractIngredient {
  ...
  public boolean test(@Nullable ItemStack stack) {
    ...
    return stack != null && !stack.isEmpty() && FluidCapabilityHelper.item(stack).flatMap(cap -> {
      ...
      if (cap.getTanks() == 1) {
        ...
        if (!contained.isEmpty() && fluidIngredient.getAmount(contained.getFluid()) == contained.getAmount() && fluidIngredient.test(contained.getFluid())) {
          ...
          return FluidCapabilityHelper.item(copy);
        }
      }
  ...
  public JsonElement toJson() {
    ...
    if (display != null) {
      json.add("display", Ingredient.CODEC.encodeStart(JsonOps.INSTANCE, display).result().orElseGet(JsonObject::new));
    }
    return json;
  }
  ...
  private static class Serializer implements IIngredientSerializer<FluidContainerIngredient> {
    ...
    public FluidContainerIngredient parse(JsonObject json) {
      ...
      if (json.has("display")) {
        display = CraftingHelper.getIngredient(JsonHelper.getElement(json, "display"), false);
      }
    ...
    public FluidContainerIngredient parse(FriendlyByteBuf buffer) {
      ...
      if (buffer.readBoolean()) {
        display = Ingredient.CONTENTS_STREAM_CODEC.decode((net.minecraft.network.RegistryFriendlyByteBuf) buffer);
      }
      return new FluidContainerIngredient(fluidIngredient, display);
    }
    ...
    public void write(FriendlyByteBuf buffer, FluidContainerIngredient ingredient) {
      ...
      if (ingredient.display != null) {
        ...
        Ingredient.CONTENTS_STREAM_CODEC.encode((net.minecraft.network.RegistryFriendlyByteBuf) buffer, ingredient.display);
      } else {
