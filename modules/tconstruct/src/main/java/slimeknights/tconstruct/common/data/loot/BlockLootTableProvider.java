...
public class BlockLootTableProvider extends BlockLootSubProvider {
  /** Copied from vanilla {@link BlockLootSubProvider} as the field is private in 1.21 */
  ...
  private void addWorld() {
    ...
    this.add(TinkerWorld.slimeLeaves.get(FoliageType.ENDER), leaves -> droppingSilkOrShears(leaves,
      applyExplosionDecay(leaves, LootItem.lootTableItem(TinkerCommons.slimeball.get(SlimeType.ENDER)).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))))
        .when(BonusLevelTableCondition.bonusLevelFlatChance(fortune(), LEAVES_STICK_CHANCES))));
  ...
  private LootItemCondition.Builder silkTouchCondition() {
  ...
  private LootItemCondition.Builder silkTouchOrShearsCondition() {
    return HAS_SHEARS.or(silkTouchCondition());
  }
  ...
  private LootTable.Builder droppingSilkOrShears(Block block, LootPoolEntryContainer.Builder<?> alternativeLootEntry) {
    return createSelfDropDispatchTable(block, silkTouchOrShearsCondition(), alternativeLootEntry);
  }

  /** Reimplementation of {@link #createLeavesDrops(Block, Block, float...)} dropping the sticks from the loot table */
  private LootTable.Builder dropSapling(Block leaves, Block sapling, float... saplingChances) {
    return droppingSilkOrShears(leaves, applyExplosionCondition(leaves, LootItem.lootTableItem(sapling))
      .when(BonusLevelTableCondition.bonusLevelFlatChance(fortune(), saplingChances)));
  }

  private LootTable.Builder randomDropSlimeBallOrSapling(FoliageType foliageType, Block leaves, Block sapling, float... saplingChances) {
    LootTable.Builder builder = dropSapling(leaves, sapling, saplingChances);
    SlimeType slime = foliageType.asSlime();
    if (slime != null) {
      return builder.withPool(
        LootPool.lootPool().setRolls(ConstantValue.exactly(1))
                .when(silkTouchOrShearsCondition().invert())
