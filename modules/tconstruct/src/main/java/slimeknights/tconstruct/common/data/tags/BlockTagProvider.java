...
public class BlockTagProvider extends BlockTagsProvider {
  ...
  private void addCommon() {
    ...
    addGlass(TinkerCommons.clearStainedGlass, "glass/", tag(Tags.Blocks.DYED));
    addGlass(TinkerCommons.clearStainedGlassPane, "glass_panes/", tag(Tags.Blocks.DYED));
  ...
  private void addHarvest() {
    ...
    TinkerWorld.slimeDirt.forEach((type, block) -> this.tag(CommonHooks.getTagFromVanillaTier(type.getHarvestTier())).add(block));
    for (DirtType dirt : DirtType.values()) {
      for (FoliageType grass : FoliageType.values()) {
        this.tag(CommonHooks.getTagFromVanillaTier((Tiers) HarvestTiers.max(dirt.getHarvestTier(), grass.getHarvestTier())))
            .add(TinkerWorld.slimeGrass.get(dirt).get(grass));
      }
    }
