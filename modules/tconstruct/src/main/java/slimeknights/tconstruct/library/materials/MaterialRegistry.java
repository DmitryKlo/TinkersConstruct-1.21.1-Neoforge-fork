...
public final class MaterialRegistry {
  ...
  public static TagSource<IMaterial> getTagSource() {
    return new TagSource<>() {
      @Override public ResourceKey<? extends Registry<IMaterial>> key() { return MaterialManager.REGISTRY_KEY; }
      @Override public String folder() { return "tinkering/materials"; }
      @Override public boolean hasTag(TagKey<IMaterial> tag) { return false; }
      @Override public Stream<TagKey<IMaterial>> tagKeys() { return Stream.empty(); }
      @Override public List<IMaterial> valuesInTag(TagKey<IMaterial> tag) { return null; }
      @Override public List<ResourceLocation> keysInTag(TagKey<IMaterial> tag) { return null; }
      @Override public IMaterial getValue(ResourceLocation key) { return MaterialRegistry.getMaterial(new MaterialId(key)); }
      @Override public Stream<TagKey<IMaterial>> tagsFor(IMaterial value) { return Stream.empty(); }
      @Override public Stream<ResourceLocation> valueKeys() { return MaterialRegistry.getMaterials().stream().map(IMaterial::getIdentifier).map(MaterialId::getLocation); }
    };
  }
