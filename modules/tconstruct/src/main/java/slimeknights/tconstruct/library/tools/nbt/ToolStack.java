...
public class ToolStack implements IToolStackView {
  ...
  public ItemStack updateStack(ItemStack stack, boolean copyNBT) {
    ...
    if (copyNBT) {
      StackDataHelper.setTag(stack, nbt.copy());
    } else {
      StackDataHelper.setTag(stack, nbt);
    }
    // ensure the damage value is set on the stack for the sake of stacking, since bypassing the vanilla setter skips that
    CompoundTag stackTag = StackDataHelper.getOrCreateTag(stack);
    if (!stackTag.contains(TAG_DAMAGE, Tag.TAG_ANY_NUMERIC) && stack.getItem().isDamageable(stack)) {
      stackTag.putInt(TAG_DAMAGE, 0);
      StackDataHelper.setTag(stack, stackTag);
    }
    return stack;
  }
  ...
  public boolean isSameStack(ItemStack stack) {
    ...
    return nbt == StackDataHelper.getTag(stack) && (stack.isEmpty() || stack.getItem() == item);
  }
