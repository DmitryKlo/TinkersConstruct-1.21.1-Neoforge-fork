...
public abstract class AbstractMaterialTraitDataProvider extends GenericDataProvider {
   ...
   * @param traits    Traits to add
   */
  protected void addDefaultTraits(MaterialId location, ResourceLocation... traits) {
    material(location).addDefaultTraits(traits);
  }

  /**
   * Adds a mixed set of material traits for legacy datagen call sites.
   * Accepts {@link ModifierId}, {@link LazyModifier}, {@link ResourceLocation}, and {@link ModifierEntry}.
   */
  protected void addDefaultTraits(MaterialId location, Object... traits) {
    material(location).addDefaultTraits(traits);
  }
   ...
   * @param traits    Traits to add
   */
  protected void addTraits(MaterialId location, MaterialStatsId statsId, ResourceLocation... traits) {
    material(location).addTraits(statsId, traits);
  }
   ...
   * Accepts {@link ModifierId}, {@link LazyModifier}, {@link ResourceLocation}, and {@link ModifierEntry}.
   */
  protected void addTraits(MaterialId location, MaterialStatsId statsId, Object... traits) {
    material(location).addTraits(statsId, traits);
  }
  ...
  public static class MaterialTraitsBuilder {
    ...
    /** Adds the given traits to the list */
    private static void addAll(List<ModifierEntry> list, ResourceLocation[] traits) {
      for (ResourceLocation trait : traits) {
        list.add(new ModifierEntry(new ModifierId(trait), 1));
      }
    }

    /** Adds the given mixed traits to the list */
    private static void addAll(List<ModifierEntry> list, Object[] traits) {
      for (Object trait : traits) {
        if (trait instanceof ModifierEntry entry) {
          list.add(entry);
        } else if (trait instanceof ModifierId id) {
          list.add(new ModifierEntry(id, 1));
        } else if (trait instanceof LazyModifier lazy) {
          list.add(new ModifierEntry(lazy, 1));
        } else if (trait instanceof ResourceLocation location) {
          list.add(new ModifierEntry(new ModifierId(location), 1));
        } else {
          throw new IllegalArgumentException("Unsupported material trait entry: " + trait);
        }
      }
    }
    ...
    /** Adds the list of traits to the builder */
    public MaterialTraitsBuilder addDefaultTraits(ResourceLocation... traits) {
      addAll(defaultTraits, traits);
      return this;
    }

    /** Adds the list of mixed traits to the builder */
    public MaterialTraitsBuilder addDefaultTraits(Object... traits) {
      addAll(defaultTraits, traits);
      return this;
    }
    ...
    /** Adds the passed traits to the builder. */
    public MaterialTraitsBuilder addTraits(MaterialStatsId statsId, ResourceLocation... traits) {
      addAll(getList(statsId, traits.length), traits);
      return this;
    }

    /** Adds the passed mixed traits to the builder. */
    public MaterialTraitsBuilder addTraits(MaterialStatsId statsId, Object... traits) {
      addAll(getList(statsId, traits.length), traits);
      return this;
    }
