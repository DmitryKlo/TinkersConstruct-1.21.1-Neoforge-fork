...
public class SmelteryRecipeBuilder {
  ...
  private void oreMelting(float scale, String tagPrefix, @Nullable TagKey<Item> size, float factor, String output, boolean forceOptional) {
    ...
    if (oreByproducts.length == 0) {
      ...
      // multiple options, save each variant with its own conditions
      boolean alwaysPresent = false;
      for (IByproduct byproduct : oreByproducts) {
        alwaysPresent = byproduct.isAlwaysPresent();
        Consumer<FinishedRecipe> byproductConsumer = alwaysPresent
          ? withCondition(TrueCondition.INSTANCE)
          : withCondition(tagCondition("ingots/" + byproduct.getName()));
        ResourceLocation byproductLocation = alwaysPresent
          ? location
          : location(meltingFolder, output + "_with_" + byproduct.getName());
        supplier.get()
                .addByproduct(byproduct.getFluid(scale))
                .setOre(oreRate, byproduct.getOreRate())
                .save(byproductConsumer, byproductLocation);
        if (alwaysPresent) {
          break;
        }
      }
