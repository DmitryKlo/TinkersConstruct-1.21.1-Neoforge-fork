...
public class ShapedMaterialRecipe extends ShapedRecipe {
  ...
  public ShapedMaterialRecipe(ShapedRecipe recipe, List<MaterialVariantId> extraMaterials) {
    this(TConstruct.getResource("wrapped"), recipe, extraMaterials);
  }

  public ShapedMaterialRecipe(ResourceLocation id, ShapedRecipe recipe, List<MaterialVariantId> extraMaterials) {
    this(id, recipe.getGroup(), recipe.category(), recipe.getWidth(), recipe.getHeight(), recipe.getIngredients(), recipe.getResultItem(null), recipe.showNotification(), extraMaterials);
  }
  ...
  public MaterialValueIngredient getMaterial() {
    if (material == null) {
      ...
      for (Ingredient ingredient : getIngredients()) {
        ...
        if (AbstractIngredient.unwrap(ingredient) instanceof MaterialValueIngredient materialValue) {
          if (material == null) {
  ...
  public static class Serializer implements LoggingRecipeSerializer<ShapedMaterialRecipe> {
    ...
    public ShapedMaterialRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
      ShapedRecipe base = RecipeSerializer.SHAPED_RECIPE.codec().codec().parse(JsonOps.INSTANCE, ShapedMaterialsRecipe.Serializer.normalizeVanillaRecipeJson(json)).getOrThrow(IllegalStateException::new);
      ShapedMaterialRecipe recipe = new ShapedMaterialRecipe(recipeId, base, MATERIAL_FIELD.get(json));
    ...
    public ShapedMaterialRecipe fromNetworkSafe(ResourceLocation recipeId, FriendlyByteBuf buffer) {
      ...
      return new ShapedMaterialRecipe(recipeId, recipe, extraMaterials);
    }
