...
public class ToolModel implements IUnbakedGeometry<ToolModel> {
  ...
  private static void addModifierQuads(Function<Material, TextureAtlasSprite> spriteGetter, ModifierModelMap modifierModels, List<FirstModifier> firstModifiers, boolean showTraits, IToolStackView tool, Consumer<Collection<BakedQuad>> quadConsumer, @Nullable ItemLayerPixels pixels, Transformation transforms, boolean isLarge) {
    if (!modifierModels.isEmpty()) {
      ...
      if (!modifiers.isEmpty()) {
        ...
        for (int i = modifiers.size() - 1; i >= 0; i--) {
          ModifierEntry entry = modifiers.get(i);
          ModifierId modifier = entry.getModifier().getModifierId();
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker, Function<Material,TextureAtlasSprite> spriteGetter, ModelState modelTransform, ItemOverrides overrides) {
    ...
    ResourceLocation modelLocation = ResourceLocation.parse(modelName);
  ...
  public static final class MaterialOverrideHandler extends NestedOverrides {
    ...
    public BakedModel resolve(BakedModel originalModel, ItemStack stack, @Nullable ClientLevel world, @Nullable LivingEntity entity, int seed) {
      ...
      if (ammoKey != null && persistentData.contains(ammoKey, Tag.TAG_COMPOUND)) {
        ClientLevel level = world != null ? world : Minecraft.getInstance().level;
        ammo = level != null ? ItemStack.parseOptional(level.registryAccess(), persistentData.getCompound(ammoKey)) : ItemStack.EMPTY;
        builder.add(ammo.getItem());
        CompoundTag tag = StackDataHelper.getTag(ammo);
        if (tag != null) {
          builder.add(tag);
        }
      } else {
