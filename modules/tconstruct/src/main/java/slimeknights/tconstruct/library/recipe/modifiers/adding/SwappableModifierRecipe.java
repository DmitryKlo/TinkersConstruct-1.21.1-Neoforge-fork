...
public class SwappableModifierRecipe extends ModifierRecipe {
  ...
  public RecipeResult<LazyToolStack> getValidatedResult(ITinkerStationContainer inv, RegistryAccess access) {
    ...
    ResourceLocation modifierKey = modifier.getLocation();
    ...
    if (level > 0 && tool.getPersistentData().getString(modifierKey).equals(value)) {
    ...
    persistentData.putString(modifierKey, value);
  ...
  public interface VariantFormatter {
    ...
    VariantFormatter MATERIAL = LOADER.register(getResource("material"), (modifier, variant) -> {
      MaterialVariantId material = MaterialVariantId.tryParse(variant);
      if (material == null) {
        return Component.literal(variant);
      }
      return Component.translatable(Util.makeTranslationKey("material", material.getLocation('.')));
    });
