...
public class MoldingRecipeBuilder extends AbstractRecipeBuilder<MoldingRecipeBuilder> {
  ...
  private class Finished extends AbstractFinishedRecipe {
    ...
    public void serializeRecipeData(JsonObject json) {
      json.add("material", Ingredient.CODEC.encodeStart(JsonOps.INSTANCE, material).getOrThrow(IllegalStateException::new).getAsJsonObject());
      if (pattern != Ingredient.EMPTY) {
        json.add("pattern", Ingredient.CODEC.encodeStart(JsonOps.INSTANCE, pattern).getOrThrow(IllegalStateException::new).getAsJsonObject());
        if (patternConsumed) {
