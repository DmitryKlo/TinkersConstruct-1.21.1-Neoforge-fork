...
public class ModifiableCrossbowItem extends ModifiableLauncherItem {
  ...
  public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
    ...
    if (heldAmmo.isEmpty()) {
      ...
      if (!ammo.isEmpty() || tool.getModifiers().has(TinkerTags.Modifiers.CHARGE_EMPTY_BOW_WITH_DRAWTIME)) {
        ...
        if (!ammo.isEmpty()) {
          if (storeDrawingItem) {
            persistentData.put(KEY_DRAWBACK_AMMO, ammo.save(RegistryAccess.EMPTY, new CompoundTag()));
          } else {
  ...
  public static void fireCrossbow(IToolStackView tool, LivingEntity living, boolean creative, InteractionHand hand, CompoundTag heldAmmo) {
    ...
    if (!level.isClientSide) {
      ...
      ItemStack ammo = ItemStack.parseOptional(RegistryAccess.EMPTY, heldAmmo);
  ...
  public void releaseUsing(ItemStack bow, Level level, LivingEntity living, int chargeRemaining) {
    ...
    if (!ammo.isEmpty()) {
      ...
      if (!level.isClientSide) {
        CompoundTag ammoNBT = (CompoundTag)ammo.save(RegistryAccess.EMPTY, new CompoundTag());
  ...
  public List<Component> getStatInformation(IToolStackView tool, @Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
    ...
    if (!heldAmmo.isEmpty()) {
      ItemStack heldStack = ItemStack.parseOptional(RegistryAccess.EMPTY, heldAmmo);
