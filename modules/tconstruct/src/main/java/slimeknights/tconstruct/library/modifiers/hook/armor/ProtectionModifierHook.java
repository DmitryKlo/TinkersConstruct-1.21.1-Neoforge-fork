...
public interface ProtectionModifierHook {
  ...
  static float getProtectionCap(@Nullable TinkerDataCapability.Holder data) {
    return Math.min(20 + (data != null ? data.get(TinkerDataKeys.PROTECTION_CAP) : 0f), 25 * 0.95f);
  }

  /** Gets the maximum protection amount on the given entity */
  @SuppressWarnings("removal")
  static double getProtectionCap(LivingEntity living, @Nullable TinkerDataCapability.Holder data) {
    return Math.min(living.getAttributeValue(BuiltInRegistries.ATTRIBUTE.wrapAsHolder(TinkerAttributes.PROTECTION_CAP.get())) * 25f + (data != null ? data.get(TinkerDataKeys.PROTECTION_CAP) : 0f), 25 * 0.95f);
  }
