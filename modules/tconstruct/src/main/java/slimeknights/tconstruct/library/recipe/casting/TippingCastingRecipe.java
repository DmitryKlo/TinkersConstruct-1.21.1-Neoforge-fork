...
public class TippingCastingRecipe extends PotionCastingRecipe {
  ...
  public boolean matches(ICastingContainer inv, Level level) {
    ...
    if (super.matches(inv, level) && ModifierUtil.getModifierLevel(stack, modifier) > 0) {
      ...
      return fluidTag != null && fluidTag.contains("Potion", Tag.TAG_STRING)
        && !ModifierUtil.getPersistentString(stack, modifier.getLocation()).equals(fluidTag.getString("Potion"));
    }
    return false;
  }
  ...
  public ItemStack assemble(ICastingContainer inv, HolderLookup.Provider access) {
    ...
    if (tag != null) {
      ToolStack.from(result).getPersistentData().putString(modifier.getLocation(), tag.getString("Potion"));
    }
    return result;
  }
      ...
      displayRecipes = BuiltInRegistries.POTION.stream()
        ...
        .map(potion -> {
          ...
          return new DisplayCastingRecipe(getId(), getType(), tools, fluid.getFluids().stream()
            .map(fluid -> PotionHelper.potionFluid(fluid.getFluid(), fluid.getAmount(), fluidNBT))
            .toList(),
