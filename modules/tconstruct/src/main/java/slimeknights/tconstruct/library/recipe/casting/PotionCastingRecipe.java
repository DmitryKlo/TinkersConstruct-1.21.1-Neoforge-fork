...
public class PotionCastingRecipe implements ICastingRecipe, IMultiRecipe<DisplayCastingRecipe> {
  ...
  public ItemStack assemble(ICastingContainer inv, HolderLookup.Provider access) {
    ...
    var potion = PotionHelper.getPotion(inv.getFluidTag());
    if (potion != null) {
      PotionHelper.setPotion(result, potion);
    }
    return result;
  }
  ...
  public List<DisplayCastingRecipe> getRecipes(RegistryAccess access) {
    if (displayRecipes == null) {
      ...
      displayRecipes = BuiltInRegistries.POTION.stream()
        ...
        .map(potion -> {
          ItemStack result = PotionHelper.setPotion(new ItemStack(this.result), potion);
          return new DisplayCastingRecipe(getId(), getType(), bottles, fluid.getFluids().stream()
                                                              .map(fluid -> PotionHelper.potionFluid(fluid.getFluid(), fluid.getAmount(), PotionHelper.toTag(result)))
                                                              .toList(),
