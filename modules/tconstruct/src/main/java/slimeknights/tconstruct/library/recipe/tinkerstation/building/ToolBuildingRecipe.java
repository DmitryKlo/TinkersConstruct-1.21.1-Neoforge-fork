...
public class ToolBuildingRecipe implements ITinkerStationRecipe {
  ...
  public RecipeResult<LazyToolStack> getValidatedResult(ITinkerStationContainer inv, RegistryAccess access) {
    ...
    ItemStack renderTool = output instanceof IModifiableDisplay display ? display.getRenderTool() : output.asItem().getDefaultInstance();
    return LazyToolStack.success(tool, Math.min(renderTool.getMaxStackSize(), count));
  }
      ...
      if (!this.materials.isEmpty()) {
        ...
        if (offset < materialCount) {
          ...
          if (offset == 0) {
            ...
            net.minecraft.nbt.CompoundTag tag = StackDataHelper.getTag(result);
            if (tag == null) {
              tag = new net.minecraft.nbt.CompoundTag();
            }
            tag.putBoolean(TooltipUtil.KEY_DISPLAY, true);
            StackDataHelper.setTag(result, tag);
          }
        }
      }
  ...
  public ItemStack assemble(ITinkerStationContainer inv, HolderLookup.Provider access) {
    return getValidatedResult(inv, RegistryAccess.EMPTY).getResult().getStack();
  }
}
