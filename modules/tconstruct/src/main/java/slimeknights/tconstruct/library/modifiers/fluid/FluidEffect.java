...
public interface FluidEffect<C extends FluidEffectContext> extends IHaveLoader, UnloadableFluidEffect<C> {
  ...
  FluidEffect<FluidEffectContext.Block> WEATHER = simple((fluid, level, context, action) -> {
    ...
    if (level.isFull() && state.getBlock() instanceof WeatheringCopper) {
      Block nextBlock = WeatheringCopper.NEXT_BY_BLOCK.get().get(state.getBlock());
      if (nextBlock != null && action.execute() && context.getLevel() instanceof ServerLevel world) {
        world.setBlockAndUpdate(context.getBlockPos(), nextBlock.withPropertiesOf(state));
      }
      return nextBlock != null ? 1 : 0;
    }
    return 0;
  });
