...
public class JEIPlugin implements IModPlugin {
  ...
  public void registerVanillaCategoryExtensions(IVanillaCategoryExtensionRegistration registry) {
    // TODO 1.21: JEI removed the old crafting category extension entry point used here.
    // Re-enable material crafting displays once the new API equivalent is wired.
  }
  ...
  private static void addCastingCatalyst(IRecipeCatalystRegistration registry, ItemLike item, mezz.jei.api.recipe.RecipeType<IDisplayableCastingRecipe> ownCategory, RecipeType<MoldingRecipe> type) {
    ...
    // TODO 1.21: RecipeManager#byType is no longer public; restore conditional molding catalyst detection.
  }
  ...
  public void registerRecipeCatalysts(IRecipeCatalystRegistration registry) {
    ...
    for (Holder<Item> item : BuiltInRegistries.ITEM.getTagOrEmpty(TinkerTags.Items.MODIFIABLE)) {
      if (item.value() instanceof IModifiableDisplay modifiable) {
        ...
        if (traits.getLevel(TinkerModifiers.severing.getModifierId()) > 0) {
        ...
        if (traits.getLevel(TinkerModifiers.melting.getModifierId()) > 0) {
  ...
  public void registerItemSubtypes(ISubtypeRegistration registry) {
    ...
    // potions
    registry.registerSubtypeInterpreter(VanillaTypes.ITEM_STACK, TinkerFluids.potion.asItem(), (PotionSubtypeInterpreter<ItemStack>) PotionHelper::toTag);
    registry.registerSubtypeInterpreter(NeoForgeTypes.FLUID_STACK, TinkerFluids.potion.get(), (stack, context) -> "");
  ...
  public void registerGuiHandlers(IGuiHandlerRegistration registration) {
    // TODO 1.21: update tank overlays to JEI's current screen handler bounds.
  }
  ...
  public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration) {
    ...
    // TODO 1.21: ToolInventoryTransferInfo no longer matches the JEI transfer registration overload.
  }
  ...
  public void onRuntimeAvailable(IJeiRuntime jeiRuntime) {
    ...
    if (level != null) {
      ...
      if (!easterEggs.isEmpty()) {
        // TODO 1.21: hideRecipes now expects recipe holders; restore easter egg hiding after JEI port.
      }
    }
