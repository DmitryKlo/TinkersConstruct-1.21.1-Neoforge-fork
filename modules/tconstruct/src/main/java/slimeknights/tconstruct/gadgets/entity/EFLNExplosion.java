      ...
      for (int y = -range; y < range; ++y) {
        for (int z = -range; z < range; ++z) {
          ...
          if (distance <= radiusSq) {
            BlockPos blockpos = new BlockPos(x, y, z).offset(Mth.floor(center.x), Mth.floor(center.y), Mth.floor(center.z));
            // no air blocks
            ...
            float strength = radius() * (1f - distance / radiusSq);
            ...
            if (source != null) {
              power = source.getBlockExplosionResistance(this, this.level, blockpos, blockstate, fluid, power);
            }

            strength -= (power + 0.3F) * 0.3F;

            if (strength > 0.0F && (source == null || source.shouldBlockExplode(this, this.level, blockpos, blockstate, strength))) {
              set.add(blockpos);
            }
          }
        }
