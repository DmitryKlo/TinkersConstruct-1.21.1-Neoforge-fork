...
public class ModifierClientEvents {
  ...
  public static void renderHotbar(RenderGuiLayerEvent.Post event) {
    ...
    if (mc.options.hideGui || event.getName() != VanillaGuiLayers.HOTBAR || player == null || player != mc.getCameraEntity()) {
      return;
    }
    ...
    if (playerController != null && playerController.getPlayerMode() != GameType.SPECTATOR) {
      ...
      if (renderShield) {
        ...
        graphics.renderItem(nextOffhand, x, y);
      }
      ...
      if (renderSleeves) {
        ...
        graphics.renderItem(currentSleeve, x, y);
      }
      ...
      if (!map.isEmpty() && mc.level != null) {
        ...
        net.minecraft.world.level.saveddata.maps.MapId index = null;
      ...
      if (renderItemFrame) {
        ...
        for (int r = 0; r < lastRow; r++) {
          for (int c = 0; c < columns; c++) {
            graphics.renderItem(itemFrames.get(i), xStart + c * SLOT_BACKGROUND_SIZE, yStart + r * SLOT_BACKGROUND_SIZE);
            i++;
          }
        }
        // align last row
        for (int c = 0; c < inLastRow; c++) {
          graphics.renderItem(itemFrames.get(i), xStart + c * SLOT_BACKGROUND_SIZE + lastRowOffset, yStart + lastRow * SLOT_BACKGROUND_SIZE);
          i++;
        }
      }
    }
