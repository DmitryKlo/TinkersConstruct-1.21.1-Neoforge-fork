...
public class CombatFishingHookRenderer extends EntityRenderer<CombatFishingHook> {
  ...
  private static void stringVertex(float x, float y, float z, VertexConsumer consumer, PoseStack.Pose pose, float first, float second) {
    float px = x * first;
    float py = y * (first * first + first) * 0.5F + 0.25F;
    float pz = z * first;
    float nx = x * second - px;
    float ny = y * (second * second + second) * 0.5F + 0.25F - py;
    float nz = z * second - pz;
    float length = Mth.sqrt(nx * nx + ny * ny + nz * nz);
    nx /= length;
    ny /= length;
    nz /= length;
    consumer.addVertex(pose, px, py, pz).setColor(0, 0, 0, 255).setNormal(pose, nx, ny, nz);
  }
