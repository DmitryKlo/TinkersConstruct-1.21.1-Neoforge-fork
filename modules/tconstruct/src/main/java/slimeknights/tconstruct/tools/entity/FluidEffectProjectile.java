...
public class FluidEffectProjectile extends Projectile implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  private IItemHandlerModifiable getCannonInventory() {
    ...
    if (this.cannon != null && level.isLoaded(this.cannon)) {
      ...
      if (cannonBE != null) {
        IItemHandler handler = level.getCapability(Capabilities.ItemHandler.BLOCK, cannonBE.getBlockPos(), cannonBE.getBlockState(), cannonBE, null);
  ...
  protected void addAdditionalSaveData(CompoundTag nbt) {
    ...
    if (cannon != null) {
      nbt.putLong(KEY_CANNON, cannon.asLong());
    }
    FluidStack fluid = getFluid();
    if (!fluid.isEmpty()) {
      nbt.put(KEY_FLUID, fluid.save(RegistryAccess.EMPTY, new CompoundTag()));
    }
  }
  ...
  protected void readAdditionalSaveData(CompoundTag nbt) {
    ...
    if (nbt.contains(KEY_CANNON)) {
      this.cannon = BlockPos.of(nbt.getLong(KEY_CANNON));
    } else {
      this.cannon = null;
    }
    setFluid(FluidStack.parseOptional(RegistryAccess.EMPTY, nbt.getCompound(KEY_FLUID)));
  }
}
