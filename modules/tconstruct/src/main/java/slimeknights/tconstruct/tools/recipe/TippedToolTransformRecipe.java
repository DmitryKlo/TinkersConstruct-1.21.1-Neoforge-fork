      ...
      if (tool.getModifierLevel(modifier) > 0) {
        ...
        if (!stack.isEmpty()) {
          ...
          if (tag != null && tag.contains("Potion", Tag.TAG_STRING)) {
            tool.getPersistentData().putString(modifier.getLocation(), tag.getString("Potion"));
          }
        }
      }
    }
      ...
      displayOutput = Arrays.stream(ingredients.get(0).getItems())
        .map(stack -> {
          ...
          if (tag != null) {
            ...
            ToolStack.from(copy).getPersistentData().putString(modifier.getLocation(), tag.getString("Potion"));
            return copy;
          }
          return result;
        }).toList();
