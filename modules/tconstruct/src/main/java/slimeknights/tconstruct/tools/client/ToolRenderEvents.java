...
public class ToolRenderEvents {
  ...
  static void renderBlockHighlights(RenderHighlightEvent.Block event) {
    ...
    do {
      ...
      if (world.getWorldBorder().isWithinBounds(pos)) {
        rendered++;
        LevelRenderer.renderLineBox(matrices, vertexBuilder, world.getBlockState(pos).getShape(world, pos).bounds().move(pos).move(-x, -y, -z), 0, 0, 0, 0.4f);
      }
  ...
  static void renderBlockDamageProgress(RenderLevelStageEvent event) {
    ...
    VertexConsumer vertexBuilder = vertices.getBuffer(ModelBakery.DESTROY_TYPES.get(progress));
