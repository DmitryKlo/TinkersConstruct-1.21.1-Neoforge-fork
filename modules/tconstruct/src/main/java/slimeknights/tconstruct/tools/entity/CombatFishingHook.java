...
public class CombatFishingHook extends FishingHook implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  private int tconstructStuckLife = 0;
        ...
        if (target.hurt(source, damage)) {
          if (!this.level().isClientSide && owner instanceof LivingEntity ownerLiving) {
            if (targetLiving != null) {
              // TODO 1.21: restore post-hurt enchantment hooks with ServerLevel damage context.
            }
  ...
  private void pullGrapple(Entity owner) {
    ...
    if (isDrill() && owner instanceof Player player) {
      player.startAutoSpinAttack(20, 8.0F, ItemStack.EMPTY);
    }
  ...
  public void tick() {
    ...
    int oldLife = this.tconstructStuckLife;
    super.tick();
    if (this.wallState != null && !level().isClientSide) {
      this.tconstructStuckLife = oldLife + 1;
      if (this.tconstructStuckLife >= 1200) {
        this.discard();
      }
    }
  }
