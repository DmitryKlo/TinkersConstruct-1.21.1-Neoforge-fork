...
public class ToolContainerMenu extends AbstractContainerMenu {
  ...
  public static ToolContainerMenu forClient(int id, Inventory inventory, FriendlyByteBuf buffer) {
    ...
    if (syncType == ToolSyncType.FULL_STACK) {
      stack = buffer instanceof RegistryFriendlyByteBuf registryBuffer ? ItemStack.STREAM_CODEC.decode(registryBuffer) : ItemStack.EMPTY;
    ...
    if (StackDataHelper.hasTag(stack) && stack.is(TinkerTags.Items.MODIFIABLE)) {
      IItemHandler handlerCap = stack.getCapability(Capabilities.ItemHandler.ITEM);
      handler = handlerCap instanceof IItemHandlerModifiable modifiable ? modifiable : EmptyItemHandler.INSTANCE;
    ...
    if (craftingContainer != null && resultContainer != null) {
      if (!player.level().isClientSide && player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
        ...
        ItemStack result = player.level().getServer().getRecipeManager()
                                ...
                                .map(holder -> holder.value().assemble(input, player.level().registryAccess()))
                                .filter(stack -> stack.isItemEnabled(player.level().enabledFeatures()))
                                .orElse(ItemStack.EMPTY);
        resultContainer.setItem(0, result);
        setRemoteSlot(0, result);
        serverPlayer.connection.send(new net.minecraft.network.protocol.game.ClientboundContainerSetSlotPacket(containerId, incrementStateId(), 0, result));
      }
    }
  }
