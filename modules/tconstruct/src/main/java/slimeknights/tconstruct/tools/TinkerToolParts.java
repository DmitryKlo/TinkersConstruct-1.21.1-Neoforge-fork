...
public final class TinkerToolParts extends TinkerModule {
  ...
  // armor
  public static final EnumObject<ArmorItem.Type,ToolPartItem> plating = ITEMS.registerEnum(ModifiableArmorMaterial.HUMANOID_TYPES, "plating", type -> new ToolPartItem(ITEM_PROPS, PlatingMaterialStats.TYPES.get(type.ordinal()).getId()));
  ...
  private static void addTabItems(ItemDisplayParameters itemDisplayParameters, CreativeModeTab.Output tab) {
    ...
    for (ArmorItem.Type type : ModifiableArmorMaterial.HUMANOID_TYPES) {
