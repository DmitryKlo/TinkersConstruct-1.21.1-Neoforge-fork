...
public enum ZoomModule implements ModifierModule, GeneralInteractionModifierHook, KeybindInteractModifierHook, UsingToolModifierHook, EquipmentChangeModifierHook {
  ...
  private static void stopZoom(ModifierEntry modifier, @Nullable TinkerDataCapability.Holder data) {
    if (data != null) {
      data.computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).remove(modifier.getId().getLocation());
    }
  }
