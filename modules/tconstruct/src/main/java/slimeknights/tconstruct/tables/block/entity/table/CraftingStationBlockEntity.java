  ...
  public ItemStack calcResult(@Nullable Player player) {
    ...
    if (!this.level.isClientSide && this.level.getServer() != null) {
      ...
      if (recipe == null || !recipe.matches(input, this.level)) {
        recipe = manager.getRecipeFor(RecipeType.CRAFTING, input, this.level).map(holder -> holder.value()).orElse(null);
      }
...
//    }
  ...
  public void takeResult(Player player, ItemStack result, int amount) {
    ...
    // TODO 1.21: keep RecipeHolder in the cache so recipe book unlocks can be restored.
