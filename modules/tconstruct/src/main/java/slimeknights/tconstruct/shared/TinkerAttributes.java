...
public class TinkerAttributes {
  ...
  void addAttributes(EntityAttributeModificationEvent event) {
    // player attributes
    event.add(EntityType.PLAYER, holder(USE_ITEM_SPEED));
    event.add(EntityType.PLAYER, holder(CRITICAL_DAMAGE));
    event.add(EntityType.PLAYER, holder(MINING_SPEED_MULTIPLIER));
    event.add(EntityType.PLAYER, holder(EXPERIENCE_MULTIPLIER));
    event.add(EntityType.PLAYER, holder(JUMP_COUNT));
    // general attributes
  ...
  private static void addToAll(EntityAttributeModificationEvent event, RegistryObject<Attribute> attribute, double defaultValue) {
    Holder<Attribute> attr = holder(attribute);
