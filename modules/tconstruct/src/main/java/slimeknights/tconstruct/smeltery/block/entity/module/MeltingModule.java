...
public class MeltingModule implements IMeltingContainer, ContainerData {
  ...
  private IMeltingRecipe findRecipe() {
    ...
    // if that fails, try to find a new recipe. The 1.21 RecipeManager cached lookup can miss
    // custom melting inputs, so scan the registered melting type directly.
    Optional<IMeltingRecipe> newRecipe = world.getRecipeManager().getAllRecipesFor(TinkerRecipeTypes.MELTING.get()).stream()
  ...
  public CompoundTag writeToTag() {
    ...
    if (!stack.isEmpty()) {
      stack.save(RegistryAccess.EMPTY, nbt);
