...
public class TankItem extends BlockTooltipItem {
  ...
  /** Registers fluid capabilities for all tank items */
  public static void registerCapabilities(RegisterCapabilitiesEvent event) {
    var fluidHandler = Capabilities.FluidHandler.ITEM;
    Consumer<Item> register = item -> {
      if (item instanceof TankItem tankItem) {
        event.registerItem(fluidHandler, (stack, ctx) -> new TankItemFluidHandler(tankItem, stack), item);
      }
    };
    TinkerSmeltery.searedTank.forEach(item -> register.accept(item.asItem()));
    TinkerSmeltery.scorchedTank.forEach(item -> register.accept(item.asItem()));
    register.accept(TinkerSmeltery.searedCastingTank.get().asItem());
    register.accept(TinkerSmeltery.searedLantern.asItem());
    register.accept(TinkerSmeltery.scorchedLantern.asItem());
  }
  ...
  private static void removeTank(ItemStack stack) {
    ...
    if (nbt != null) {
      ...
      if (nbt.isEmpty()) {
        StackDataHelper.setTag(stack, null);
      }
    }
  }
  ...
  public static ItemStack setTank(ItemStack stack, FluidTank tank) {
    if (tank.isEmpty()) {
      ...
      nbt.put(NBTTags.TANK, tank.writeToNBT(RegistryAccess.EMPTY, new CompoundTag()));
  ...
  public static FluidTank getTank(ItemStack stack, int scale) {
    ...
    CompoundTag nbt = StackDataHelper.getTag(stack);
    if (nbt != null) {
      tank.readFromNBT(RegistryAccess.EMPTY, nbt.getCompound(NBTTags.TANK));
    }
    return tank;
  }
